...
var _component = require("./transformers/component.js");
 ...
 */
function simplifyRawFigmaObject(apiResponse, nodeExtractors) {
  ...
  var _extractFromDesign = (0, _nodeWalker.extractFromDesign)(rawNodes, nodeExtractors, options, globalVars),
  ...
  return _objectSpread(_objectSpread({}, metadata), {}, {
    ...
    components: (0, _component.simplifyComponents)(components),
    componentSets: (0, _component.simplifyComponentSets)(componentSets),
...
function parseAPIResponse(data) {
  ...
  if ("nodes" in data) {
    ...
    nodesToParse = nodeResponses.map(function (n) {
      return n.document;
    }).filter(_common.isVisible);
  } else {
    // GetFileResponse
    Object.assign(aggregatedComponents, data.components);
    Object.assign(aggregatedComponentSets, data.componentSets);
    nodesToParse = data.document.children.filter(_common.isVisible);
  }
  var name = data.name,
